...
export default class Books {
    ...
    async GetBooksFromPublisher(page: number = 1, limit: number = 5): Promise<bookType[]> {
        const skip = (page - 1) * limit;
        try {
            const books = await this.#books.find({ tag: "Publish" }).sort({ _id: -1 }).skip(skip).limit(limit).exec();
            return books || [];
        } catch (error) {
    ...
    async GetBooksFromQuestion(page: number = 1, limit: number = 5): Promise<bookType[]> {
        const skip = (page - 1) * limit;
        try {
            const books = await this.#books.find({ tag: "Question" }).sort({ _id: -1 }).skip(skip).limit(limit).exec();
            return books || [];
        } catch (error) {
